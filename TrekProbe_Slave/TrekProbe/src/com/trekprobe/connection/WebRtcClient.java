package com.trekprobe.connection;
...
public class WebRtcClient {
	...
	public WebRtcClient(RTCListener listener, String host, Context context) {
		mListener = listener;
		...
		SocketIOClient.connect(host, new ConnectCallback() {
			...
			public void onConnectCompleted(Exception ex, SocketIOClient socket) {
				...
				client.addListener("actuator_protocol", messageHandler);
			}
		}, new Handler());
		...
		soundPool.load(context, R.raw.smw_jump, 1);
		soundPool.load(context, R.raw.smw_back, 1);
	}
	...
	public void sendMessage(String to, String type, String name, JSONObject payload) throws JSONException {
	...
	private class MessageHandler implements EventCallback {
		...
		public void onEvent(String s, JSONArray jsonArray, Acknowledge acknowledge) {
			...
			try {
				...
				} else if(s.equals("actuator_protocol")){
					JSONObject json = jsonArray.getJSONObject(0);

					JSONObject payload = json.getJSONObject("payload");
					String msg = payload.getString("msg");
					
					Log.d("PROTOCOL MSG", msg);
	...
	private class Peer implements SdpObserver, PeerConnection.Observer{
		...
		public void onCreateSuccess(final SessionDescription sdp) {
			try {
				...
				sendMessage(id, sdp.type.canonicalForm(), Variables.droidName, payload);
		...
		public void onIceCandidate(final IceCandidate candidate) {
			try {
				...
				sendMessage(id, "candidate", Variables.droidName, payload);
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}
	...
	public void setCamera(String cameraFacing, String height, String width){
		...
		videoConstraints.mandatory.add(new MediaConstraints.KeyValuePair("minHeight", height));
